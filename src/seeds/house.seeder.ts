...
import { HousePayment } from "src/house-payments/entities/installmentPayment";
...
export class HouseSeeder {
    ...
    async seedHouses() {
        ...
        for (const house of houses) {
            const existingHouse = await this.housePaymentRepository.findOne({ where: { deltaNumber: house.deltaNumber } });
            if (!existingHouse) {
                await this.housePaymentRepository.save({ ...house, paidAmount: 0, isFullyPaid: false });
            }
        }
